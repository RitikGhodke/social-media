...
//                                     <div className={`flex ${isSender ? "justify-end" : "justify-start"} mb-2 group relative`}>
//                                         {!isSender && (
//                                             <img
//                                                 src={userData?.profilePicture || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"}
//                                                 className="w-7 h-7 rounded-full object-cover mr-2 self-end flex-shrink-0"
...
const ChatBox = () => {
    ...
    const [replyingTo, setReplyingTo] = useState(null)
    const [showReactions, setShowReactions] = useState(null)
    const [swipeX, setSwipeX] = useState({})        // ✅ swipe state
    const [swipeStart, setSwipeStart] = useState({}) // ✅ swipe start
    const myUserData = useSelector(store => store.user)
    const nav = useNavigate()
    const messagesEndRef = useRef(null)
    const typingTimeout = useRef(null)
    ...
    async function btnClickHandler() {
        ...
        try {
            const res = await axios.post(
                import.meta.env.VITE_DOMAIN + "/api/chats/send",
                {
                    receiver: id,
                    text,
                    image: imageBase64,
            ...
            // ✅ Socket se sirf notify karo
            socket.current.emit("new-msg-notify", {
                sender: myUserData._id,
                receiver: id,
                msgId: newMsg._id
            })
    ...
    async function handleReact(msgId, emoji) {
        try {
            const res = await axios.patch(
                `${import.meta.env.VITE_DOMAIN}/api/chats/message/${msgId}/react`,
                { emoji },
                { withCredentials: true }
            )
    ...
    function handleTouchMove(e, item) {
        ...
        // Sender swipes left, receiver swipes right
    ...
    function handleTouchEnd(e, item) {
        ...
        if (Math.abs(swipe) > 40) {
            setReplyingTo(item) // ✅ Reply set karo
        }
        // Reset swipe
