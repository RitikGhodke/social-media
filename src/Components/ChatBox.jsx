...
//                         {/* 
//                             textarea use kiya instead of input:
//                             - Chrome ka autofill/password/payment strip SIRF <input> pe aata hai
//                             - <textarea> pe yeh strip kabhi nahi aata — permanent fix
//                             - rows=1 aur resize-none se single line jesa dikhta hai
//                             - onKeyDown me Enter=send, Shift+Enter=newline
//                         */}
//                         <textarea
//                             rows={1}
...
//                                     e.preventDefault()
//                                     if (!selectedImage) btnClickHandler()
//                                 }
//                             }}
//                             autoComplete="off"
//                             autoCorrect="off"
...
const ChatBox = () => {
    ...
    useEffect(() => {
        ...
        socket.current.on("user-online", (userId) => {
            if (userId?.toString() === id?.toString()) setIsOnline(true)
        })

        socket.current.on("user-offline", (userId) => {
            if (userId?.toString() === id?.toString()) setIsOnline(false)
        })
        ...
        <div className="fixed inset-0 flex flex-col bg-gray-100">
            ...
            <div className="flex flex-1 overflow-hidden min-h-0">
                ...
                <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                    ...
                    {/* ✅ Header — bilkul fixed, kabhi nahi hilega */}
                    <div className="relative flex-shrink-0 flex items-center px-4 h-16 bg-white border-b border-gray-200 shadow-sm z-40">
                                ...
                                <>
                                    <div className="relative flex-shrink-0">
                                        <img
                                            ...
                                            alt="profile"
                                        />
                                        {/* ✅ Green dot sirf tab dikhega jab actually online ho */}
                                        {isOnline && (
                                            <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full ring-2 ring-white"></span>
                                        )}
                                    </div>
                                    <div className="min-w-0">
                                        ...
                                        {/* ✅ Online/typing text real status se */}
                                        <p className={`text-xs ${isTyping || isOnline ? "text-green-500" : "text-gray-400"}`}>
                    ...
                    {/* ✅ Input Bar — keyboard window nahi aayega */}
