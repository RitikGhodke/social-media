...
//                         ) : conv.length > 0 ? (
//                             conv.map((item, index) => (
//                                 <ConvCard key={index} data={item} myId={myData._id} />
//                             ))
//                         ) : (
//                             <div className="flex flex-col items-center justify-center py-16 text-gray-400">
//                                 <MessageCircle size={48} className="mb-3 text-gray-300" />
...
const Chats = () => {
    ...
    useEffect(() => {
        ...
        socket.on("conversation-updated", ({ sender, receiver, lastMsg }) => {
            setConv(prev => {
                const updated = prev.map(c => {
                    const isMatch =
                        (c.sender._id === sender && c.receiver._id === receiver) ||
                        (c.sender._id === receiver && c.receiver._id === sender)

                    if (isMatch) return { ...c, lastMsg, updatedAt: new Date().toISOString() }
                    return c
                })
